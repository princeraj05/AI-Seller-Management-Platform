...
export default function FlipkartChannel() {
  const [connection, setConnection] = useState(null);
  const [loading, setLoading] = useState(true);
  const [actionLoading, setActionLoading] = useState(false);
  const [message, setMessage] = useState(null);
  const [showConnectModal, setShowConnectModal] = useState(false);
  const [formData, setFormData] = useState({ displayName: 'Flipkart Seller Account', externalAccountId: '' });
  ...
  const fetchConnection = async () => {
    ...
    try {
      ...
      if (res && res.data && Array.isArray(res.data.channels)) {
        const conn = res.data.channels.find((c) => c.provider === 'FLIPKART' && c.status === 'CONNECTED');
        setConnection(conn || null);
      } else {
        setConnection(null);
      }
    } catch (err) {
      console.warn('Error fetching Flipkart connection:', err.message);
      setConnection(null);
    } finally {
      setLoading(false);
    }
  ...
  const handleSyncNow = async () => {
    ...
    if (!connection || connection.status !== 'CONNECTED') {
      setMessage({ type: 'error', text: 'Flipkart is not connected.' });
      return;
    }
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Flipkart synchronization completed successfully.' });
      await fetchConnection();
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Failed to sync Flipkart.' });
    } finally {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Flipkart channel disconnected successfully.' });
      await fetchConnection();
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Failed to disconnect Flipkart.' });
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleConnectSubmit = async (e) => {
    ...
    setActionLoading(true);
    try {
      await connectChannelApi('FLIPKART', {
        displayName: formData.displayName || 'Flipkart Seller Account',
        externalAccountId: formData.externalAccountId,
        credentials: { appAppId: 'flipkart-app-id' },
      });
      setMessage({ type: 'success', text: 'Flipkart account connected successfully!' });
      setShowConnectModal(false);
      await fetchConnection();
    } catch (err) {
      setMessage({ type: 'error', text: err.message || 'Failed to connect Flipkart account.' });
    } finally {
      setActionLoading(false);
    }
  };
    ...
    <div className="space-y-6 font-sans text-xs">
        ...
        <div className="flex items-center gap-2">
          {isConnected ? (
            <>
              ...
              >
                <LogOut className="w-4 h-4" /> Disconnect Flipkart
              </button>
              <button
                onClick={handleSyncNow}
                disabled={actionLoading}
                className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded-lg flex items-center gap-1.5 shadow-sm disabled:opacity-50"
              >
                <RefreshCw className={`w-4 h-4 ${actionLoading ? 'animate-spin' : ''}`} /> Sync Flipkart Now
              </button>
            </>
          ) : (
            <button
              onClick={() => setShowConnectModal(true)}
              className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded-lg flex items-center gap-1.5 shadow-sm"
            >
              <PlusCircle className="w-4 h-4" /> Connect Flipkart Account
            </button>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4">
        <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
          <div className="w-12 h-12 rounded-xl bg-blue-500 text-white font-extrabold text-xl flex items-center justify-center">
            F
          </div>
          <div>
            <h1 className="text-xl font-bold text-slate-900">Flipkart Seller Account</h1>
            <p className="text-slate-500">
              {isConnected
                ...
                : 'Flipkart Seller Account • Not Connected'}
            </p>
          </div>
          {isConnected ? (
        ...
        {!isConnected && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 mt-4">
            <div>
              <h3 className="font-bold text-blue-900">Connect Your Flipkart Seller Account</h3>
              <p className="text-blue-700 text-xs mt-0.5">
                Link your Flipkart Seller Hub account to synchronize listings, inventory, and order fulfillment.
              </p>
            </div>
            <button
              onClick={() => setShowConnectModal(true)}
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs px-4 py-2 rounded-lg whitespace-nowrap shadow-sm"
            >
      ...
      {showConnectModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl space-y-4">
            <h2 className="text-lg font-bold text-slate-900">Connect Flipkart Seller Hub</h2>
            <form onSubmit={handleConnectSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Flipkart Official Store"
                  required
                />
              </div>
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Flipkart Seller ID</label>
                <input
                  type="text"
                  value={formData.externalAccountId}
                  onChange={(e) => setFormData({ ...formData, externalAccountId: e.target.value })}
                  className="w-full border border-slate-300 rounded-lg p-2 text-xs"
                  placeholder="e.g. FK-SELLER-1234"
                  required
                />
              </div>
